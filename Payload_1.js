  ...
  while (i < bytes.length) { //Keep going until we've read all the payload bytes.
    ...
    switch (type) {
      ...
      case 104: // LPP_RELATIVE_HUMIDITY
       ...
       //PAR value
      case 112: // LPP_SPECTRAL   //currently this will send an array of 24 bytres of raw data
        let intVal = bytestoInt(bytes.slice(i, i+4));
        data[`spectral_data`] = {
          values: intVal,
          unit: "raw"
        };
...
function bytestoInt(bytes){
  ...
  return view.getInt32(0, false); // Reads 4 bytes starting at byte offset 0 as a signed int32
}
