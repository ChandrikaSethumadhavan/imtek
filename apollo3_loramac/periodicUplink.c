...
void periodicUplink( void )
{
    ...
    while( 1 )
    {
        ...
        CRITICAL_SECTION_END( );
				...
				#elif HW_CF_SENSOR
				APP_TX_DUTYCYCLE = 300000;		//Can change this to change duty cycle when needed or wanted
				#endif
				TxPeriodicity = 0;
...
static void PrepareTxFrame( void )
{
		...
		//JalapenosLppAddDummy ( );
		//todo: define the device ID somewhere general
		JalapenosLppAddDeviceID(1000);
		#if defined(HW_CF_SENSOR)
			JalapenosLppAddPAM(CHANNEL_PAM, data.Measurement.PAM_F, data.Measurement.PAM_Fm, data.Measurement.PAMYield);
			JalapenosLppAddSpectral(CHANNEL_SPECTRAL, data.Measurement.spectral_bytes);
